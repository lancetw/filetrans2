            ...
            if (FD_ISSET(fd, &testfds)) {
                ...
                if (fd == server_sockfd) {
                    ...
                    if (nread == 0) {        /* 客戶端沒資料或是斷線 */
                        ...
                        /* 依客戶端的要求切換模式 (Download/Upload) */
                        
                        st = recv(fd, &mode, sizeof (char), 0);
                        ...
                        if (mode == 'D') {    /* 下載模式 */
                            ...
                            for (i = 0, cread = 1, per = 0; i < nread; i = i + 1, cread = cread + 1) {
                                ...
                                cls(); 
                                printf("\r進度：%3d %c %8s 正在提供下載 \"%s\" %d / %d bytes\n", per, '%', dot[j], filename, cread, nread);
